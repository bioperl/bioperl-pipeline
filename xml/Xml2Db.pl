...
foreach my $iohandler ($xso1->child('pipeline_setup')->child('iohandler_setup')->children('iohandler')) {
  ...
  # iohandler_type
  my $iotype;
  if(defined($iohandler->child('iohandler_type'))){
      $iotype = $iohandler->child('iohandler_type')->value;
  }
  if(exists($iohandler_attrs{'type'})){
      $iotype = $iohandler->attribute("type");
  }
  ...
  foreach my $method ($iohandler->children('method')) {
    ...
    # name
    my $name;
    if(defined $method->child('name')){
        $name = $method->child('name')->value;
    }
    if(exists $method_attrs{'name'}){
        $name = $method_attrs{'name'};
    }

    # rank
    my $rank;
    if(defined $method->child('rank')){
        $rank = $method->child('rank')->value;
    }
    if(exists $method_attrs{'rank'}){
        $rank = $method_attrs{'rank'};
    }
    ...
    foreach my $argument (@arg) {
      if(ref($argument)){ #overcome bug in SimpleObj
        my %argument_attrs = $argument->attributes;
        
        # tag
        my $tag;
        if (defined $argument->child('tag')) { 
          $tag = $argument->child('tag')->value;
        }
        if(exists $argument_attrs{'tag'}){
            tag = $argument_attrs{'tag'};
        }
        
        # value  
        my $value;
        if(defined $argument->child('value')){
            $value= $argument->child('value')->value;
        }
        if(exists $argument_attrs{'value'}){
            $value = $argument_attrs{'value'};
        }
        
        # rank
        my $rank;
        if(defined $argument->child('rank')){
            $rank = $argument->child('rank')->value;
        }
        if(exists $argument_attrs{'rank'}){
            $rank = $argument_attrs{'rank'};
        }
        
        # type
        my $type;
        if(defined $argument->child('type')){
            $type = $argument->child('type')->value;
        }
        if(exists $argument_attrs{'type'}){
            $type = $argument_attrs{'type'};
        }
...
#              my $converter_obj = _get_converter($converter->attribute("id"));
 #             if(defined($converter_obj)){
  ...
  #               print "converter for analysis  not found\n";
 #             }
            my $converter_id = $converter->attribute("id");
	    my $converter_module = $converter->attribute("module");
            my $converter_obj = new Bio::Pipeline::Converter(
		-dbID => $converter_id,
		-module => $converter_module );
		
	    $dba->get_ConverterAdaptor->store($converter_obj);
            }
...
foreach my $rule ($xso1->child('pipeline_setup')->child('pipeline_flow_setup')->children('rule')) {
 if(ref($rule)) {
   my %rule_attrs = $rule->attributes;
   
   my $current_analysis_id;
   if(defined $rule->child('current_analysis_id')){
       $current_analysis_id = $rule->child('current_analysis_id')->value;
   }
   if(exists $rule_attrs{'current'}){
       $current_analysis_id = $rule_attrs{'current'};
   }
   my $current = _get_analysis($current_analysis_id);
   if (!defined($current)) {
     print "current analysis not found for rule\n";
   }
   
   my $next_analysis_id;
   if(defined $rule->child('next_analysis_id')){
       $next_analysis_id = $rule->child('next_analysis_id')->value;
   }
   if(exists $rule_attrs{'next'}){
       $next_analysis_id = $rule_attrs{'next'};
   }
   my $next = _get_analysis($next_analysis_id);
   if (!defined($next)) {
     print "next analysis not found for rule\n";
   }
   my $action;
   if(defined $rule->child('action')){
       $action = $rule->child('action')->value;
   }
   if(exists $rule_attrs{'action'}){
